! function(a) {
  ...
  b = function() {
    function b(b, d) {
      var e = this;
      this.element = b, this.dataStore = d.dataStore, this.start = c(this.start, this), this.stop = c(this.stop, this), this.complete = c(this.complete, this), this.undo = c(this.undo, this), a(document).on("touchstart", this.element, this.start), a(document).on("touchend", this.element, this.stop), this.element.mouseenter(this.start), this.element.mouseleave(this.stop), this.element.click(this.undo), this.render(), this.$counter = this.element.find(".count .num"), this.dataStore.hasVoted().then(function(a) {
        a && e.element.addClass("complete")
      }), this.dataStore.onKudoUpdates(function(a) {
        e.setCount(a)
      })
    }
    document.location.pathname;
    return b.prototype.render = function() {
      var b = a('<a class="kudobject"><div class="opening"><div class="circle">&nbsp;</div></div></a><a href="#kudo" class="count"><span class="num">0</span><span class="txt">Kudos</span></a>');
      this.element.append(b)
    }, b.prototype.start = function() {
      savedCount = parseInt(this.$counter.html(), 10), this.isKudoed() || (this.$counter.html("Don't"), this.element.find(".count .txt").html("Move"), this.element.addClass("active"), this.activationTimer = setTimeout(this.complete, 1e3))
    }, b.prototype.stop = function() {
      this.isKudoed() || (this.$counter.html(savedCount), this.element.find(".count .txt").html("Kudos")), clearTimeout(this.activationTimer), this.element.removeClass("active")
    }, b.prototype.complete = function() {
      this.stop(), this.dataStore.addKudo(), this.element.addClass("complete"), toast("Thanks, for liking the post!", 4e3)
    }, b.prototype.undo = function() {
      this.isKudoed() && (this.dataStore.removeKudo(), savedCount = parseInt(this.$counter.html(), 10), this.element.removeClass("complete"), toast('Any <a class="yellow-text" style="padding-left:4px;padding-right:4px;" href="#" onclick=\'$("html, body").animate({ scrollTop: $(document).height() }, 10);\'>Comments</a> or <a class="yellow-text" style="padding-left:4px;padding-right:4px;" href="/about" target="_blank">Feedback</a> for disliking ?', 8e3))
    }, b.prototype.currentCount = function() {
      var a = parseInt(this.$counter.html(), 10);
      return isNaN(a) && (a = 0), a
    }, b.prototype.isKudoed = function() {
      return this.element.hasClass("complete")
    }, b.prototype.setCount = function(a) {
      this.$counter.html(a)
    }, b
  }(), a(function() {
    a.fn.kudoable = function(c) {
  ...
  var b = new Firebase("https://softvar-blog-kudos.firebaseio.com/"),
    c = b.child("kudos"),
    d = document.location.pathname.replace(/[\/-]/g, "").replace(".html", ""),
    e = function() {
      var a = $.Deferred(),
        c = b.getAuth();
      return null == c ? b.authAnonymously(function(b, c) {
        a.resolve(c)
      }) : a.resolve(c), a.promise()
    };
  "" === d && (d = "localhost");
  var f = function() {
      var a = $.Deferred();
      return e().then(function(b) {
        c.child(d).child("likes").child(b.uid).once("value", function(b) {
          a.resolve(null !== b.val())
        })
      }), a.promise()
    },
    g = function() {
      e().then(function(a) {
        c.child(d).child("likes").child(a.uid).set({
          count: 1
        })
      })
    },
    h = function() {
      e().then(function(a) {
        c.child(d).child("likes").child(a.uid).remove()
      })
    },
    i = function(a) {
      c.child(d).on("value", function(b) {
        if (b) {
          var c = b.val(),
            d = 0;
          if (c)
            for (var e in c.likes) d++;
          a(d)
        }
      })
    },
...
function(a) {
  ...
  function(a) {
    ...
    function b(a) {
      for (var b, c, d = [], e = 0, f = a.length; f > e;) b = a.charCodeAt(e++), b >= 55296 && 56319 >= b && f > e ? (c = a.charCodeAt(e++), 56320 == (64512 & c) ? d.push(((1023 & b) << 10) + (1023 & c) + 65536) : (d.push(b, c), e--)) : d.push(b);
      return d
    }

    function c(a, b) {
      var c = a && ("[object NodeList]" === Object.prototype.toString.call(a) && a.length || 1 === a.nodeType),
        d = b && "function" == typeof b;
      return "console" in window && "warn" in console && (c || console.warn("Countable: No valid elements were found"), d || console.warn('Countable: "' + b + '" is not a valid callback function')), c && d
    }
    ...
    function e(a, c) {
      var d, e = "value" in a ? a.value : a.innerText || a.textContent;
      return c.stripTags && (e = e.replace(/<\/?[a-z][^>]*>/gi, "")), c.ignoreZeroWidth && (e = e.replace(/[\u200B]+/, "")), d = e.trim(), {
        paragraphs: d ? (d.match(c.hardReturns ? /\n{2,}/g : /\n+/g) || []).length + 1 : 0,
        sentences: d ? (d.match(/[.?!…]+./g) || []).length + 1 : 0,
        words: d ? (d.replace(/['";:,.?¿\-!¡]+/g, "").match(/\S+/g) || []).length : 0,
        characters: d ? b(d.replace(/\s/g, "")).length : 0,
        all: b(c.ignoreReturns ? e.replace(/[\n\r]/g, "") : e).length
      }
    }
    ...
    var g = [],
      h = "oninput" in document ? "input" : "keyup";
        ...
        var j = d(i),
          k = function(a) {
            ...
            g.push({
              element: a,
              handler: c
            }), c(), a.addEventListener ? a.addEventListener(h, c, !1) : a.attachEvent && a.attachEvent("on" + h, c)
          };
        if (c(a, b)) return a.length ? f(a, k) : k(a), this
      },
      die: function(a) {
        return c(a, function() {}) ? (f(a, function(a) {
          var b;
          f(g, function(c) {
            c.element === a && (b = c)
          }), b && (a.removeEventListener ? a.removeEventListener(h, b.handler, !1) : a.detachEvent && a.detachEvent("on" + h, b.handler), g.splice(g.indexOf(b), 1))
        }), this) : void 0
      },
      once: function(a, b, g) {
        return c(a, b) ? (f(a, function(a) {
          b.call(a, e(a, d(g)))
        }), this) : void 0
      },
      count: function(a, b, c) {
        return this.once(a, b, c)
      },
      enabled: function(a) {
        var b = !1;
        return a && 1 === a.nodeType && f(g, function(c) {
          c.element === a && (b = !0)
        }), b
      }
    };
